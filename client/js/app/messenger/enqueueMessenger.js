...
define([
    ...
    return function(html) {
        ...
        if (chatInfo.right.length > 0) { //pulls in a window that is overflowed off to the right if exists
            ...
            _.first(chatInfo.right).width(_.remove(chatInfo.center, function(n) {
                return n.is(html);
            })[0].width());
            chatInfo.center.push(_.remove(chatInfo.right, function(n) {
            ...
            //reconfigure chat info
            _.last(chatInfo.left).width(_.remove(chatInfo.center, function(n) {
                return n.is(html);
            })[0].width());
            chatInfo.center.unshift(_.remove(chatInfo.left, function(n) {
